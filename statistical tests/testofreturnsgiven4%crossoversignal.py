...
for x in counter1:
        for y in counter2:
            ...
            for file in glob.glob('*.json'):
             ...
             Vdivergence=V/Vlong-1
             a=0
             while a<ll+hour:
                 try:
                     vtrigger[a]=np.percentile(Vlong[a-240:a], 99.9, axis=0)
             ...
             while i<ll:
                 ...
                 if Pdivergence[i]>=ptrigger[i] and ptrigger[i]!=0:#SLPdivergence[i]>0.051+x*0.001: #checking if price criterion was met                      
                   if V[i]>=vtrigger[i]: #checking if volume criterion was met.
                       try:
                           ...
                           SP.append(Apdivergence[0])
                           SV.append(Avdivergence[0])
