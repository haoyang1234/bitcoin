...
# prefil an dataframe 
Var2 = np.vstack((Po[c-1+ma+pre:],Po30[pre+ma-c:],Po90[pre+c:],dPoc
                  [c-1+ma+pre:], V[c-1+ma+pre:],V30[ma-c+pre:],N[c-1+ma+pre:],N30[ma-c+pre:])). T  
df2 = pd.DataFrame(Var2,index=df['Time'][c-1+ma+pre:],columns=['P','P30','P90','dPoc','V','V30','N','N30'])
ll=len(df2.index)
signal0=np.zeros(ll) #signal for Pclose-Popen. 
...
#create signals
while i<ll:
    vtrigger=np.percentile(V30[1+i:pre+i], 99.9, axis=0) #volume trigger 
    ntrigger=np.percentile(N30[pre-Npre+1+i:pre+i], 99.9, axis=0) #number of trades trigger
    if df2["V"][i]>vtrigger:
        signal1[i]=1 #volume trigger 
    if df2["N"][i]>ntrigger:
        signal4[i]=1 #number of trades trigger
    if df2["P"][i]>df2["P30"][i]:
        signal2[i]=1 #moving average cross over 
    buytheo[i]=2*df2["P"][i]-df2["P30"][i]
    selltheo[i]=2*df2["P"][i]-df2["P90"][i]
    i += 1
...
combo=signal1*signal2*signal4#*signal5*signal3
...
#if anomoly buy in the first 3 minute half the volume or until all 10 btc are exhausted. 
#define exit signal 
